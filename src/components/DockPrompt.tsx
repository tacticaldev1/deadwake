...
const DockPrompt: React.FC<DockPromptProps> = ({ village, onDock }) => {
  return (
    <div className="absolute left-1/2 bottom-24 -translate-x-1/2 z-20 animate-fade-in">
      <button
        onClick={onDock}
        className="pixel-border bg-primary text-primary-foreground px-6 py-3 font-display text-[10px] pointer-events-auto hover:bg-primary/80 transition-colors pixel-btn"
      >
        [E] DOCK AT {village.name.toUpperCase()}
      </button>
      <div className="text-center mt-2">
        <p className="font-body text-sm text-muted-foreground/80">{village.description}</p>
      </div>
    </div>
  );
};
